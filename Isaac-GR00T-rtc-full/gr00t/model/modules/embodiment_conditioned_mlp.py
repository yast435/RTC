...
class MultiEmbodimentActionEncoder(nn.Module):
    ...
    def forward(self, actions, timesteps, cat_ids):
        ...
        Args:
            ...
            timesteps: [B,] or [B, T] timesteps.
                Standard flow matching passes a single scalar per batch item (B,).
                Training-Time RTC passes per-token timesteps (B, T) where prefix
                tokens have t=1.0 (clean) and suffix tokens have t=random.
            cat_ids: [B,] category/embodiment IDs
        Returns:
        ...
        # Accept both (B,) scalar timesteps and (B, T) per-token timesteps.
        if timesteps.dim() == 1 and timesteps.shape[0] == B:
            # Standard: shape (B,) => (B, T)
            timesteps = timesteps.unsqueeze(1).expand(-1, T)
        elif timesteps.dim() == 2 and timesteps.shape == (B, T):
            # Training-Time RTC: per-token timesteps already provided
            pass
        else:
            raise ValueError(
