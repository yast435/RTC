...
class FinetuneConfig:
    ...
    # --- Training-Time RTC ---
    training_rtc_max_latency: int = 0
    """
    Maximum simulated latency in action steps for Training-Time RTC.
    When set to a value > 0, Training-Time RTC (Real-Time Chunking) is
    automatically enabled. The model simulates inference latency during
    training by freezing a random prefix of the action chunk as known
    conditions, forcing the model to predict only the remaining suffix.
