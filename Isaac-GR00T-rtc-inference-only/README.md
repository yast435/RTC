...
In the Euler integration step, both the base velocity field `pred_velocity` and the guidance correction `w * grad` are treated as velocity components. Therefore, they must both be multiplied by the time step `dt` to compute the displacement.
