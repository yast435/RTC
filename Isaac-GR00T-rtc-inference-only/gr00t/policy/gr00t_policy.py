...
class Gr00tPolicy(BasePolicy):
    ...
    def _get_action(
        ...
        if options is not None:
            rtc = options.get("rtc", None)
            if rtc is not None and rtc.get("enabled", True):
        ...
        if frozen_prefix is not None:
            frozen_prefix = torch.as_tensor(frozen_prefix, device=self.model.device)
            # dtype: use model dtype (bf16) for internal sampler consistency
            frozen_prefix = frozen_prefix.to(dtype=self.model.dtype)
        ...
        # Return normalised predictions so that RTC wrappers can build the
        # frozen prefix for the *next* inference call.
        info = {"normalized_action_pred": normalized_action.cpu().numpy()}
